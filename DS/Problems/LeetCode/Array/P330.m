...
- (NSInteger)maxPatch:(NSArray<NSNumber *> *)nums target:(NSInteger)target
{
  NSInteger max = 0, patch = 0;
int i = 0;
  while(max < target)
  {
    if(i < nums.count && nums[i].integerValue <= max + 1)
    {
      max = max + nums[i].integerValue;
      i++;
    }
    else
    {
